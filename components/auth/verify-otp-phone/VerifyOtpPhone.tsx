// components/auth/verify-otp-phone/VerifyOtpPhone.tsx
"use client";
...
export const VerifyOtpPhone = () => {
  const router = useRouter();
  const otpForm = useForm<VerifyOtpPhoneInput>({
    resolver: zodResolver(VerifyOtpPhoneSchema),
  });
  const {
    register,
  ...
  const onSubmit = async (data: VerifyOtpPhoneInput) => {
    try {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-40 backdrop-blur-sm z-50 flex items-center justify-center">
      <div className="bg-linear-to-br from-white to-gray-50 rounded-2xl shadow-2xl w-full max-w-md mx-4 relative">
        ...
        <div className="p-6">
          <form
            onSubmit={otpForm.handleSubmit(onSubmit, (errors) =>
          ...
          >
            ...
            >
              {isSubmitting ? "در حال ارسال ..." : "تایید کد"}
            </button>
          </form>
          ...
          <div className="flex justify-between pt-4 w-[90%]">
            ...
            <Link href={"/request-otp-phone"} className="text-blue-500">
