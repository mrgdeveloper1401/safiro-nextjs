      ...
      if (isAxiosError(error)) {
        ...
        if (status === 400) {
          registerForm.setError("root", {
            message: "کاربر از قبل وجود دارد، می‌توانید وارد حساب خود شوید",
          });
        } else if (status === 500) {
          registerForm.setError("root", { message: "خطای سرور" });
        } else {
          registerForm.setError("root", {
        ...
        registerForm.setError("root", { message: "خطای ناشناخته" });
      }
    }
  };
    ...
    <div className="fixed inset-0 bg-black bg-opacity-40 backdrop-blur-sm z-50 flex items-center justify-center">
      <div className="bg-linear-to-br from-white to-gray-50 rounded-2xl shadow-2xl w-full max-w-md mx-4 relative">
        ...
        <div className="border-b border-gray-200 p-6 text-center bg-linear-to-r from-blue-50 to-indigo-50 rounded-t-2xl">
          ...
          <h2 className="text-2xl font-bold text-gray-800">ایجاد حساب</h2>
          <p className="text-gray-600 mt-1 text-sm">برای ثبت‌نام اطلاعات زیر را وارد کنید</p>
        </div>
        ...
        <div className="p-6">
          <form onSubmit={registerForm.handleSubmit(onRegister)} className="space-y-4">
            ...
            {/* تکرار رمز عبور */}
            <div>
              <label className="block text-gray-700 text-sm font-semibold mb-2">تکرار رمز عبور</label>
              <div className="relative">
                <div className="absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none">
                  <svg className="h-5 w-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" />
          ...
          <div className="flex justify-between pt-4 w-[90%]">
            <Link href={"/login"} className="text-blue-500">ورود به حساب</Link>
