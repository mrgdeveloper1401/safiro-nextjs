...
const RideForm: React.FC<RideFormProps> = ({
  ...
  const handleTripSubmit = async (e: React.SyntheticEvent<HTMLFormElement>) => {
    ...
    // TypeScript: بعد از چک بالا origin/destination null نیستند
    if (!origin || !destination || !selectedTrip) return;
    ...
    const departureTime = new Date(`${dateStr}T${timeStr}:00`).toISOString();
    ...
    try {
      ...
      if (!res.ok) {
        const msg =
          data?.detail ||
          data?.message ||
          (Array.isArray(data) ? data.map((i: { message?: string }) => i.message).join("، ") : null) ||
          `خطای ${res.status}`;
        throw new Error(msg);
      }

      const tripId: number | undefined = data?.id;
      if (!tripId) {
        throw new Error("شناسه سفر از سرور دریافت نشد");
      }
  ...
  const handleCancelTrip = async () => {
    ...
    try {
      const res = await fetch(`/api/v1/trip/${createdTripId}/`, {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
      ...
      setCancelError(
        err instanceof Error ? err.message : "خطا در لغو درخواست"
      );
    } finally {
  ...
  // ---------- حالت انتظار راننده ----------
  if (viewMode === "waiting" && createdTripId) {
    return (
      <div className="overflow-hidden rounded-2xl border border-purple-100 bg-white shadow-xl">
        ...
        <div className="space-y-6 p-5 sm:p-6">
          <div className="rounded-2xl border border-emerald-100 bg-emerald-50/70 px-5 py-6 text-center">
            <div className="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-emerald-100 text-emerald-600">
              <CheckCircle className="h-8 w-8" />
            </div>
            <h3 className="text-base font-bold text-emerald-800 sm:text-lg">
              منتظر بمانید تا راننده‌ای درخواست را تایید کند
            </h3>
            <p className="mt-2 text-sm leading-7 text-emerald-700/80">
              درخواست سفر شما با موفقیت ثبت شد. به‌محض قبول شدن توسط راننده،
              جزئیات سفر برای شما نمایش داده می‌شود.
            </p>
          </div>

          {(origin || destination) && (
            <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
              <div className="rounded-xl border border-green-100 bg-green-50/40 p-3.5">
                <div className="mb-1 text-xs font-bold text-green-700">مبدا</div>
                <div className="text-sm leading-6 text-gray-700">
                  {origin?.address || "—"}
                </div>
              </div>
              <div className="rounded-xl border border-red-100 bg-red-50/40 p-3.5">
                <div className="mb-1 text-xs font-bold text-red-700">مقصد</div>
                <div className="text-sm leading-6 text-gray-700">
                  {destination?.address || "—"}
                </div>
              </div>
          ...
          {cancelError && (
            <div className="rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">
              <AlertCircle className="mb-1 inline h-5 w-5" /> {cancelError}
            </div>
          )}
          ...
          <button
            ...
            className="flex w-full items-center justify-center gap-2 rounded-xl border-2 border-red-200 bg-red-50 px-6 py-3.5 text-sm font-bold text-red-700 transition hover:bg-red-100 disabled:cursor-not-allowed disabled:opacity-70"
          >
            {cancelLoading ? (
              <>
      ...
      <form onSubmit={handleTripSubmit} className="space-y-7 p-5 sm:p-6">
        ...
        {tripMode === "reserve" ? (
          ...
          <div className="rounded-xl border border-emerald-100 bg-emerald-50/60 px-4 py-3 text-xs leading-6 text-emerald-700">
            سفر به‌صورت <span className="font-bold">فوری</span> ثبت می‌شود.
            زمان فعلی سیستم به‌عنوان زمان درخواست در نظر گرفته می‌شود.
          </div>
        )}
        ...
        {submitStatus === "error" && (
          <div className="rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">
            <AlertCircle className="mb-1 inline h-5 w-5" /> {submitMessage}
          </div>
        )}
        ...
        <button
          ...
          className="group relative w-full overflow-hidden rounded-xl bg-linear-to-l from-purple-600 to-indigo-600 px-6 py-4 text-base font-bold text-white shadow-lg shadow-purple-200 transition hover:from-purple-700 hover:to-indigo-700 hover:shadow-xl disabled:cursor-not-allowed disabled:opacity-70"
        >
